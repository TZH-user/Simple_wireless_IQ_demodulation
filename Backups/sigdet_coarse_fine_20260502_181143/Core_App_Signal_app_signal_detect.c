...
static void app_signal_detect_begin_scan(void)
{
  ...
  g_sigdet.scan_start_hz = APP_SIGDET_SCAN_START_HZ;
  g_sigdet.scan_stop_hz = APP_SIGDET_SCAN_STOP_HZ;
  g_sigdet.scan_step_hz = APP_SIGDET_SCAN_STEP_HZ;
  ...
  g_sigdet.status.stage = APP_SIGDET_STAGE_VPP_SCAN;
...
static void app_signal_detect_finish_scan(void)
{
  ...
  if (rise_raw < APP_SIGDET_VPP_MIN_RISE_RAW)
  {
    g_sigdet.status.carrier_present = 0U;
    g_sigdet.status.locked = 0U;
    g_sigdet.status.estimated_carrier_hz = app_signal_detect_step_freq_hz(g_sigdet.scan_start_hz,
                                                                          g_sigdet.scan_step_hz,
  ...
  else
  {
    ...
    else
    {
      ...
      {
        g_sigdet.status.estimated_carrier_hz =
  ...
  g_sigdet.status.estimated_carrier_hz = app_signal_detect_clamp_to_valid_hz(g_sigdet.status.estimated_carrier_hz);
