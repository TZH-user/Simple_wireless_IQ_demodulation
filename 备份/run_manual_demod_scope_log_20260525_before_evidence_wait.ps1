...
function Read-SerialCapture {
    ...
    try {
        ...
        Write-Host "现在触发板子进入任务；采集期间按 Enter 停止本项串口采集。" -ForegroundColor Yellow
        if ($MaxSec -gt 0) {
...
function Show-TestCase {
    ...
    Write-Host ""
    Write-Host "建议串口日志至少包含：demod: start、demod: mode set、demod_perf/demod_dac，以及本模式 debug 行。" -ForegroundColor DarkYellow
...
function Invoke-OneTestCase {
    ...
    while ($true) {
        ...
        if ($activeCaptureMode -eq "AutoSerial") {
            ...
            try {
                $captureResult = Read-SerialCapture -OutPath $capturePath -PortName $SerialPortName -PortBaudRate $BaudRate -MaxSec $AutoCaptureMaxSec -CommandToSend $StartCommand -EchoToConsole:$EchoSerialToConsole
...
if ($CaptureMode -eq "AutoSerial") {
    ...
    Write-Host "流程: 显示参数 -> 关闭串口助手并设置仪器 -> 按 Enter 开始脚本采集 -> 触发板子进入任务 -> 按 Enter 停止采集 -> 自动解析 -> 输入现象 -> 接受或重测。"
