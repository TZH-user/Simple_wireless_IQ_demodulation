...
void App_LvglUiInit(void)
{
  ...
  g_ui.calibrate_btn = App_LvglUiCreateModeButton(main_card, "校准", 292, 52, MODDETECT_RUN_CALIBRATION);
  g_ui.task_btn = App_LvglUiCreateModeButton(main_card, "任务", 390, 52, MODDETECT_RUN_TASK);
...
void App_LvglUiRefresh(void)
{
  ...
  if (st.run_mode == MODDETECT_RUN_IDLE)
  {
    App_LvglUiSetStatus(APP_UI_STATUS_IDLE);
  }
  ...
  else if (analyze_is_done() != 0U)
  {
    App_LvglUiSetStatus(APP_UI_STATUS_DONE);
  }
  else if ((analyze_is_active() != 0U) || (st.result_ready != 0U))
  {
