...
static uint32_t g_fsk_samples_per_symbol = 1U;
...
static uint32_t rx_demod_samples_per_symbol(void)
{
    uint32_t symbol_rate_hz = rx_demod_resolve_symbol_rate(g_rx_symbol_rate_hz);
    uint32_t samples_per_symbol = (g_sample_rate_hz + (symbol_rate_hz / 2U)) / symbol_rate_hz;
...
void RxDemod_Reset(void)
{
    ...
    g_fsk_sym_acc = 0;
    g_fsk_sym_count = 0U;
    g_fsk_samples_per_symbol = rx_demod_samples_per_symbol();
    g_fsk_low_est = 0;
    g_fsk_high_est = 0;
    g_fsk_threshold = 0;
    ...
    g_psk_sym_q_acc = 0;
    g_psk_sym_count = 0U;
    g_psk_samples_per_symbol = rx_demod_samples_per_symbol();
                              ...
                              uint16_t *dac_out)
{
    ...
    if (g_fsk_samples_per_symbol == 0U)
    {
        g_fsk_samples_per_symbol = rx_demod_samples_per_symbol();
    }
    ...
    for (i = 0U; i < n; ++i)
    {
        ...
        if (g_fsk_sym_count >= g_fsk_samples_per_symbol)
        {
