...
#define ANALYZE_IQ_SPECTRUM_ENABLE 0U /* 当前调制类型判断暂不需要 IQ 复数谱，关闭后可减少一次 4096 点 CFFT。 */
...
static void analyze_log_result(void)
{
    ...
    int n = snprintf(log_buf,
                     sizeof(log_buf),
                     "analyze: result center=%luHz low_if=%ldHz mode=%s mod=%luHz depth=%lupm env=%luHz/%lupm freq=%luHz/%lupm votes=%u/%u/%u\r\n",
                     ...
                     (unsigned int)g_analyze.amp_vote_count,
                     (unsigned int)g_analyze.freq_vote_count,
                     (unsigned int)g_analyze.quiet_vote_count);
...
{
    uint8_t amp_votes = g_analyze.amp_vote_count;
    uint8_t freq_votes = g_analyze.freq_vote_count;

#if (ANALYZE_LOW_IF_EST_ENABLE != 0U)
    g_analyze.low_if_hz = analyze_average_i32(g_analyze.low_if_sum_hz, g_analyze.vote_count);
#endif

    if ((amp_votes == 0U) && (freq_votes == 0U))
    {
        g_analyze.mode = ANALYZE_MODE_CW;
        g_analyze.mod_hz = 0U;
        return;
    }
    ...
    if ((amp_votes != 0U) && (freq_votes == 0U))
    {
        g_analyze.mode = ANALYZE_MODE_AM;
        g_analyze.mod_hz = analyze_average_mod_hz(g_analyze.amp_mod_sum_hz, amp_votes);
        return;
    }

    if ((amp_votes == 0U) && (freq_votes != 0U))
    {
        g_analyze.mode = ANALYZE_MODE_FM;
        g_analyze.mod_hz = analyze_average_mod_hz(g_analyze.freq_mod_sum_hz, freq_votes);
        return;
    }

    if (((uint32_t)g_analyze.env_score_pm * 1000UL) >=
        ((uint32_t)g_analyze.freq_score_pm * ANALYZE_MODE_SCORE_DOMINANCE_PM))
    {
        g_analyze.mode = ANALYZE_MODE_AM;
        g_analyze.mod_hz = analyze_average_mod_hz(g_analyze.amp_mod_sum_hz, amp_votes);
        return;
    ...
    if (((uint32_t)g_analyze.freq_score_pm * 1000UL) >=
        ((uint32_t)g_analyze.env_score_pm * ANALYZE_MODE_SCORE_DOMINANCE_PM))
    {
        g_analyze.mode = ANALYZE_MODE_FM;
        g_analyze.mod_hz = analyze_average_mod_hz(g_analyze.freq_mod_sum_hz, freq_votes);
        return;
    ...
    g_analyze.mod_hz = (amp_votes >= freq_votes) ?
                       analyze_average_mod_hz(g_analyze.amp_mod_sum_hz, amp_votes) :
                       analyze_average_mod_hz(g_analyze.freq_mod_sum_hz, freq_votes);
}
