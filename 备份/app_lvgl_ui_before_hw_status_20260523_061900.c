...
#define UI_DEVICE_TAG_WIDTH 84
#define UI_DEVICE_TAG_HEIGHT 24
...
static void App_LvglUiSetDeviceStatus(const char *text, lv_color_t bg);
...
void App_LvglUiInit(void)
{
  ...
  g_ui.device_status = lv_label_create(g_ui.detect_page);
  lv_obj_set_size(g_ui.device_status, UI_DEVICE_TAG_WIDTH, UI_DEVICE_TAG_HEIGHT);
  lv_label_set_long_mode(g_ui.device_status, LV_LABEL_LONG_MODE_CLIP);
  lv_obj_set_style_radius(g_ui.device_status, 8, 0);
  lv_obj_set_style_bg_opa(g_ui.device_status, LV_OPA_COVER, 0);
  lv_obj_set_style_text_color(g_ui.device_status, lv_color_hex(0xFFFFFF), 0);
  lv_obj_set_style_text_align(g_ui.device_status, LV_TEXT_ALIGN_CENTER, 0);
  lv_obj_set_style_text_font(g_ui.device_status, &lv_font_montserrat_14, 0);
  lv_obj_set_style_pad_top(g_ui.device_status, 3, 0);
  App_LvglUiSetDeviceStatus("INIT", lv_color_hex(0x64748B));
  lv_obj_align(g_ui.device_status, LV_ALIGN_TOP_RIGHT, -16, 10);
  ...
  g_ui.calibrate_btn = App_LvglUiCreateModeButton(main_card, "校准", 336, 42, MODDETECT_RUN_CALIBRATION);
  g_ui.task_btn = App_LvglUiCreateModeButton(main_card, "开始/重新开始", 336, 110, MODDETECT_RUN_TASK);
...
void App_LvglUiRefresh(void)
{
  ...
  if (demod_st.state == DEMOD_STATE_RUNNING)
  {
    ...
    App_LvglUiSetDeviceStatus("DEMOD", lv_color_hex(0x7C3AED));
  }
  else if (st.run_mode == MODDETECT_RUN_IDLE)
  {
    App_LvglUiSetStatus(APP_UI_STATUS_IDLE);
    App_LvglUiSetDeviceStatus("READY", lv_color_hex(0x2563EB));
  }
  else if (st.run_mode == MODDETECT_RUN_CALIBRATION)
  {
    ...
    App_LvglUiSetDeviceStatus((st.cal_done != 0U) ? "CAL OK" : "CAL", (st.cal_done != 0U) ? lv_color_hex(0x0284C7) : lv_color_hex(0xCA8A04));
  }
  else if ((st.result_ready != 0U) && (st.center_hz == 0UL))
  {
    App_LvglUiSetStatus(APP_UI_STATUS_UNLOCKED);
    App_LvglUiSetDeviceStatus("UNLOCK", lv_color_hex(0xDC2626));
  }
  else if ((analyze_is_done() != 0U) && (st.result_ready != 0U) && (st.center_hz != 0UL))
  {
    App_LvglUiSetStatus(APP_UI_STATUS_DONE);
    App_LvglUiSetDeviceStatus("DONE", lv_color_hex(0x16A34A));
  }
  else if (analyze_is_active() != 0U)
  {
    App_LvglUiSetStatus(APP_UI_STATUS_ANALYZING);
    App_LvglUiSetDeviceStatus("ANALYZE", lv_color_hex(0xEA580C));
  }
  else
  {
    App_LvglUiSetStatus(APP_UI_STATUS_SWEEP);
    App_LvglUiSetDeviceStatus("SCAN", lv_color_hex(0xCA8A04));
  }
...
static void App_LvglUiSetDeviceStatus(const char *text, lv_color_t bg)
{
  if ((g_ui.device_status == NULL) || (text == NULL))
  {
    return;
  }

  lv_label_set_text(g_ui.device_status, text);
  lv_obj_set_style_bg_color(g_ui.device_status, bg, 0);
}
