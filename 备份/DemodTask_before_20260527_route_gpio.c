                                          ...
                                          analyze_mode_t mode)
{
    ...
    if (power_delta_pm >= DEMOD_MONITOR_POWER_DELTA_PM)
    {
        score++;
        reason_mask |= 0x02UL;
    }
    if (env_delta_pm >= DEMOD_MONITOR_ENV_CV_DELTA_PM)
    {
        score++;
        reason_mask |= 0x04UL;
    }
    if (short_phase_delta_pm >= DEMOD_MONITOR_PHASE_DELTA_PM)
    {
        score++;
        reason_mask |= 0x08UL;
    }
    if (long_phase_delta_pm >= DEMOD_MONITOR_PHASE_DELTA_PM)
    {
        score++;
        reason_mask |= 0x10UL;
    }
    ...
    if (g_demod_monitor.suspect_blocks >= DEMOD_MONITOR_TRIGGER_BLOCKS)
    {
        ...
        n = snprintf(log_buf,
                     sizeof(log_buf),
                     "demod:monitor trigger monitor_score=%u trigger_reason=0x%02lX low_if_est=%ld env_cv=%lu phase_disp_short=%lu phase_disp_long=%lu baseline_valid=%u reanalyze_request_count=%lu\r\n",
                     (unsigned int)score,
                     (unsigned long)reason_mask,
                     (long)now.low_if_hz,
                     (unsigned long)now.env_cv_pm,
                     (unsigned long)now.short_phase_disp_pm,
                     (unsigned long)now.long_phase_disp_pm,
                     (unsigned int)g_demod_monitor.baseline_valid,
