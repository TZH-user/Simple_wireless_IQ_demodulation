...
#define ANALYZE_FM_RESCUE_COMB_TOL_HZ 1500U /* FM 多边带梳状峰允许偏差。 */
#define ANALYZE_FM_RESCUE_COMB_MIN_COUNT 5U /* 至少 5 个 IQ 强峰落在 low_if ± n*fmod 上，才认为是 FM 多边带。 */
...
static void analyze_log_result(void)
{
    ...
    int n = snprintf(log_buf,
                     sizeof(log_buf),
                     "analyze: result center=%luHz low_if=%ldHz mode=%s mod=%luHz depth=%lupm iq99=%luHz env_iq=%lupm env=%luHz/%lupm freq=%luHz/%lupm fsk=%luHz votes=%u/%u/%u/%u/%u/%u/%u/%u reason=%u\r\n",
...
static uint8_t analyze_ask_on_board_valid(uint8_t amp_valid)
{
    ...
    return analyze_near_u32(g_env_peak.freq_hz,
                            ANALYZE_DIGITAL_RATE_TARGET_HZ,
...
static uint8_t analyze_fsk_on_board_valid(uint8_t freq_valid)
{
    ...
    return analyze_near_u32(g_phase_peak.freq_hz,
                            ANALYZE_DIGITAL_RATE_TARGET_HZ,
                            ANALYZE_DIGITAL_RATE_TOL_HZ);
}
...
static analyze_block_decision_t analyze_classify_current_block(uint8_t amp_valid, uint8_t freq_valid)
{
    ...
    uint8_t psk_board = analyze_psk_on_board_valid();
    ...
    if ((amp_valid != 0U) && (env_tone_like != 0U))
    {
