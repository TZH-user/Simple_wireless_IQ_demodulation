...
#define APP_OCXO_CAL_FLASH_VERSION 8UL
...
static app_ocxo_cal_flash_record_v8_t g_ocxo_flash_record;
...
static uint8_t app_ocxo_cal_decode_record(const app_ocxo_cal_flash_record_v8_t *record,
                                          uint32_t *dac_mv,
                                          ...
                                          int32_t *dds_offset_hz)
{
    ...
    if ((record->version != APP_OCXO_CAL_FLASH_VERSION) ||
        (record->record_size != sizeof(app_ocxo_cal_flash_record_v8_t)) ||
        ...
        (record->crc != app_ocxo_cal_crc_bytes(record, offsetof(app_ocxo_cal_flash_record_v8_t, crc))))
    {
        return 0U;
    }
    ...
    if (ask_analog_demod_enable != NULL)
    {
        *ask_analog_demod_enable = (record->ask_analog_demod_enable != 0UL) ? 1U : 0U;
    }

    if (fsk_analog_demod_enable != NULL)
    {
        *fsk_analog_demod_enable = (record->fsk_analog_demod_enable != 0UL) ? 1U : 0U;
    }

    if (beep_ui_enable != NULL)
    {
        *beep_ui_enable = (record->beep_ui_enable != 0UL) ? 1U : 0U;
    }

    if (beep_sweep_lock_enable != NULL)
    {
        *beep_sweep_lock_enable = (record->beep_sweep_lock_enable != 0UL) ? 1U : 0U;
    }

    if (beep_analyze_done_enable != NULL)
    {
        *beep_analyze_done_enable = (record->beep_analyze_done_enable != 0UL) ? 1U : 0U;
    }
    ...
    if (ask_square_dc_shift != NULL)
    {
        if ((record->ask_square_dc_shift < APP_OCXO_CAL_ASK_SQUARE_DC_SHIFT_MIN) ||
        ...
        {
            *ask_square_dc_shift = (uint8_t)record->ask_square_dc_shift;
        }
    }

    if (ask_square_threshold_code != NULL)
    {
        if ((record->ask_square_threshold_code < APP_OCXO_CAL_ASK_SQUARE_THRESHOLD_MIN) ||
        ...
        {
            *ask_square_threshold_code = (uint16_t)record->ask_square_threshold_code;
        }
    }

    if (dds_offset_hz != NULL)
    {
        *dds_offset_hz = app_ocxo_cal_limit_dds_offset_hz(record->dds_offset_hz);
    }
...
static uint8_t app_ocxo_cal_write_flash(uint8_t update_ocxo_state)
{
    ...
    g_ocxo_flash_record.crc = app_ocxo_cal_crc_bytes(&g_ocxo_flash_record,
                                                     offsetof(app_ocxo_cal_flash_record_v8_t, crc));
    ...
    result = app_board_flash_read(APP_OCXO_CAL_FLASH_ADDR,
                                  ...
                                  sizeof(g_ocxo_flash_record));
    if ((result != APP_BOARD_FLASH_OK) ||
        (app_ocxo_cal_decode_record(&g_ocxo_flash_record, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL) == 0U))
    {
        if (update_ocxo_state != 0U)
        {
            g_ocxo_status.state = APP_OCXO_CAL_ERROR;
