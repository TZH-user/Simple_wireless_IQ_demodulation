...
#define APP_OCXO_CAL_FLASH_VERSION 7UL
...
static app_ocxo_cal_flash_record_v7_t g_ocxo_flash_record;
...
static uint8_t app_ocxo_cal_decode_record(const app_ocxo_cal_flash_record_v7_t *record,
                                          uint32_t *dac_mv,
                                          ...
                                          uint16_t *ask_square_threshold_code)
{
    ...
    if ((record->version != APP_OCXO_CAL_FLASH_VERSION) ||
        (record->record_size != sizeof(app_ocxo_cal_flash_record_v7_t)) ||
        (record->dac_mv < APP_OCXO_CAL_MIN_MV) ||
        (record->dac_mv > APP_OCXO_CAL_MAX_MV) ||
        (record->crc != app_ocxo_cal_crc_bytes(record, offsetof(app_ocxo_cal_flash_record_v7_t, crc))))
    {
        return 0U;
    }
...
static uint8_t app_ocxo_cal_write_flash(uint8_t update_ocxo_state)
{
    ...
    g_ocxo_flash_record.crc = app_ocxo_cal_crc_bytes(&g_ocxo_flash_record,
                                                     offsetof(app_ocxo_cal_flash_record_v7_t, crc));
    ...
    result = app_board_flash_read(APP_OCXO_CAL_FLASH_ADDR,
                                  ...
                                  sizeof(g_ocxo_flash_record));
    if ((result != APP_BOARD_FLASH_OK) ||
        (app_ocxo_cal_decode_record(&g_ocxo_flash_record, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL) == 0U))
    {
        if (update_ocxo_state != 0U)
        {
            g_ocxo_status.state = APP_OCXO_CAL_ERROR;
