...
/* 将候选区间转换为候选：平台取段中点，凸峰取最大值，凹谷取双肩之间最低点。 */
static uint8_t sweep_build_candidate(uint16_t start_step, uint16_t stop_step, app_sweep_candidate_t *candidate)
{
    uint16_t idx;
    uint16_t peak_step;
    ...
    else
    {
        ...
        {
            candidate->freq_hz = sweep_candidate_mid_grid_freq_hz(start_step, stop_step);
        }
        else
        {
            candidate->freq_hz = sweep_freq_at(g_sweep.stage_start_hz, g_sweep.stage_step_hz, peak_step);
        }
    }
